...
class ApiClient {
  ...
  private async request<T, P = Record<string, any>>(
    ...
    try {
      ...
      // Deduplicate in-flight requests (prevents React StrictMode double-fires)
      const dedupeKey = `${finalConfig.method || "GET"}|${url}|${typeof finalConfig.body === "string" ? finalConfig.body : ""}`;
      ...
      // Always remove from dedupe map so retries/failures don't block future identical calls
      const dedupeKey = `${fetchConfig.method || "GET"}|${this.buildURLWithParams(endpoint, params)}|${typeof fetchConfig.body === "string" ? fetchConfig.body : ""}`;
  ...
  private async prepareHeaders(
    ...
    // Tell DRF to skip BrowsableAPIRenderer overhead in local dev
  ...
  private serializeRequestBody(data?: unknown): {
    body?: BodyInit;
    isFormData: boolean;
  } {
  ...
  private handleRequestError(error: unknown, skipHandler: boolean): ApiError {
    ...
    const apiError = this.createError(
      error instanceof Error
        ? error.message
        : "A frontend error occurred before a request could be made",
      0,

...
# PLATFORM SURFACE
# ============================================================
# Controls which public-facing endpoint groups are mounted in urls.py.
#   DEVELOPER_API       — only the API-key-authenticated developer_api surface
#   DEVELOPER_DASHBOARD — only the session-authenticated developer_dashboard surface
#   ALL                 — both surfaces (default for local dev)
# Used to deploy the same codebase as two separate replicas: one for downstream
# developers consuming the API, one for the internal dashboard.
PLATFORM_SURFACE = os.getenv("PLATFORM_SURFACE", "ALL").strip().upper()
...
INSTALLED_APPS = (
    ...
    # "django.contrib.admin",   # don't need admin appp
    # Third-party
    "rest_framework",
    ...
    "corsheaders",
    "django_inlinecss",
    # "django_summernote",  # don't need admin appp
    "django_celery_beat",
    "health_check",
    "health_check.db",
    "health_check.cache",

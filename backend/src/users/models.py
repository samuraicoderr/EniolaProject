...
def _safe_remove_from_array(array: list, value) -> None:
    """Remove value from array if present, else do nothing."""
    try:
        array.remove(value)
    except ValueError:
        pass
...
class OnboardingMixin:
    ...
    def get_onboarding_flow(self) -> list:
        ...
        if self.is_email_verified:
            _safe_remove_from_array(flow, OnboardingStatus.NEEDS_EMAIL_VERIFICATION)
        if self.is_phone_number_verified:
            _safe_remove_from_array(flow, OnboardingStatus.NEEDS_PHONE_VERIFICATION)
            
        if self.social_auth.exists():
            # Social users don't need email verification.
            # return [
            #     step for step in flow
            #     # if step not in (OnboardingStatus.NEEDS_EMAIL_VERIFICATION,)
            #     if step != OnboardingStatus.NEEDS_EMAIL_VERIFICATION
            # ]
            _safe_remove_from_array(flow, OnboardingStatus.NEEDS_EMAIL_VERIFICATION)
            return flow
        ...
        #     if step != OnboardingStatus.NEEDS_PASSWORD
        # ]
        _safe_remove_from_array(flow, OnboardingStatus.NEEDS_PASSWORD)
        return flow
